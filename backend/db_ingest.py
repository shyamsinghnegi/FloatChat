...
def ensure_schema(engine, reset: bool = False) -> None:
    """Create (or, with reset=True, drop and recreate) the core tables."""
...
def ingest_file(engine, file_path: str) -> dict:
    """
    Ingest a single ARGO NetCDF file into PostgreSQL.
    Safe to re-run — profiles already present are skipped via ON CONFLICT.
    Returns a summary dict: {"ingested": int, "skipped": int, "errors": int}.
    """
    print(f"\nOpening {file_path}...")
    ...
    for i in range(num_profiles):
        try:
            ...
            # Use the real CYCLE_NUMBER from the float, not the loop index i —
            # NetCDF profiles aren't always stored in cycle order, so array
            # position is meaningless as an identifier.
...
def main():
    ...
    ensure_schema(engine, reset=args.reset)

    try:
        ingest_file(engine, args.file)
